...
void showLogo();
void displayPacketsBuffer();

void setup() {
  // PacketInfo* p1 = new PacketInfo("A0", "003", 50, 25, 0.73, 0.5,0.3, -53);
  // PacketsBuffer.push_back((*p1));
  // PacketInfo* p2 = new PacketInfo("A1", "011", 7, 24, 0.71, 0.5,0.3, -31);
  // PacketsBuffer.push_back((*p2));
  // PacketInfo* p3 = new PacketInfo("A0", "002", 52, 25, 0.73, 0.5,0.3, -51);  
  // PacketsBuffer.push_back((*p3));
  // PacketInfo* p4 = new PacketInfo("A1", "010", 6, 24, 0.70, 0.5,0.3, -33);
  // PacketsBuffer.push_back((*p4));
  // PacketInfo* p5 = new PacketInfo("A2", "005", 55, 24, 0.75, 0.5,0.3, -65);
  // PacketsBuffer.push_back((*p5));
  PacketsBuffer.push_back(PacketInfo("A0", "003", 50, 25, 0.73,0.5,0.3, -53));
  PacketsBuffer.push_back(PacketInfo("A1", "011", 7, 24, 0.71,0.5,0.3, -31));
  PacketsBuffer.push_back(PacketInfo("A0", "002", 52, 25, 0.73,0.5,0.3, -51));
  PacketsBuffer.push_back(PacketInfo("A1", "010", 6, 24, 0.70,0.5,0.3, -33));
  PacketsBuffer.push_back(PacketInfo("A2", "005", 55, 24, 0.75,0.5,0.3, -65));
...
void loop() {
  ...
  display.clear();
  displayWifi(rssi);
  displayNodeCount(4,3);
  displayBattery(70);


  Serial.println("List print before:");
  Serial.println(PacketsBuffer.front().rfid_reading);



  displayPacketsBuffer();
  
  Serial.println("List print after:");
  Serial.println(PacketsBuffer.front().rfid_reading);
...
void displayPacketsBuffer()
{
  if(PacketsBuffer.size()>0)
  {
    ...
    display.drawString(0, 35, line1_str);
    display.drawString(0, 45, line2_str);
  ...
  {
    display.drawString(0, 35, "Ready to Receive");
    display.display();
  }
  // else if(PacketsBuffer.size()>0)
  // {
  //   //print about packet received
  //   String line1_str = "Node: ";
  //   line1_str+= PacketsBuffer.front().device_name;
...
void displayNodeCount(int chickenCount, int pigCount) {
  // display.clear();
  display.setTextAlignment(TEXT_ALIGN_LEFT);
  display.setFont(ArialMT_Plain_10);
  String nStr = "Con. Nodes: ";
  nStr += String((chickenCount+pigCount), DEC);
